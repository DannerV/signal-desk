...
print("\nstate machine")
check("above reclaim -> BUY_WATCH", evaluate(CFG, snap(105, 104), RISK)["state"] == BUY_WATCH)
check("below invalidation -> BROKEN", evaluate(CFG, snap(88, 89), RISK)["state"] == BROKEN)
check("between -> WAIT", evaluate(CFG, snap(95, 95), RISK)["state"] == WAIT)
check("no data -> STALE", evaluate(CFG, None, RISK)["state"] == "STALE")

print("\nticket construction")
# CFG: risk/share = 105-90 = 15, reward/share = 120-105 = 15 -> R:R 1.0, below floor
t = evaluate(CFG, snap(105, 104), RISK)["ticket"]
...
cont = evaluate(dict(CFG, t1=135), snap(105, 104), RISK)["ticket"]
...
t2 = evaluate(generous, snap(105, 104), RISK)["ticket"]
...
print("\nguards")
violent = evaluate(generous, snap(105, 92, atr14=4.0), RISK)["ticket"]  # 13pt move / 4 ATR = 3.25x
...
gapped = evaluate(generous, snap(105, 96, low=103, atr14=8.0), RISK)["ticket"]
...
wide_t = evaluate(wide, snap(105, 104), RISK)["ticket"]
...
hold = evaluate(dict(CFG, earnings="2026-07-28", earnings_timing="pm"), snap(105, 104), RISK)
...
b = evaluate(CFG, snap(88, 91), RISK)["ticket"]
